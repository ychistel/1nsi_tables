...
if __name__ == '__main__' :
    """
    ecrire_fichier('trajet.csv',trajets)
    ecrire_fichier('inscrit.csv',inscrits)
    ecrire_fichier('reserve.csv',reservations)
    
    trajet=lire_fichier('trajet.csv')
    inscrit=lire_fichier('inscrit.csv')
    reservation=lire_fichier('reserve.csv')
    ...
    for dico in reservations:
        rep={}
        for dico1 in trajets:
            if dico1['id_trajet'] == dico['trajet'] :
                for dico2 in inscrits:
                    if dico2['id_inscrit']==dico['inscrit']:
